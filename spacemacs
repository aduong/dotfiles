...
This function should only modify configuration layer settings."
  (setq-default
   ...
   dotspacemacs-additional-packages '(gcmh)
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-gc-cons (list (* 1024 1024 1024) 0.1)
...
before packages are loaded."
  ;; ensure we use git-rebase-mode instead of helm-ls-git-rebase-todo-mode for rebases
  (delete '("/git-rebase-todo$" . helm-ls-git-rebase-todo-mode) auto-mode-alist)
  ;; gcmh parks collections in idle time. The 1GB high threshold let ~1GB of
  ;; garbage pile up between them: Emacs sets the next GC trigger to the MAX of
  ;; `gc-cons-threshold' and (gc-cons-percentage * heap), so the absolute value
  ;; binds and the 0.1 percentage never got a look in. `auto' sizes the idle
  ;; delay at 20x the last measured GC instead of a guessed 15s.
  (setq gcmh-high-cons-threshold (* 256 1024 1024)
        gcmh-idle-delay 'auto)
  (gcmh-mode 1)

  ;; --- LSP: cut per-keystroke / per-cursor-move server round-trips ----------
  ;; The api repo is ~1,950 Go packages, so each of these costs a gopls request
  ;; on every buffer change or cursor move. Workspace-scoping filters that pair
  ;; with this are generated by `emacs-lsp-filters' into private/local/.
  (setq lsp-lens-enable nil                   ; codeLens on every change
        lsp-modeline-code-actions-enable nil  ; codeAction per idle
        lsp-enable-links nil                  ; documentLink per change
        lsp-enable-on-type-formatting nil     ; near no-op for gopls
        lsp-enable-symbol-highlighting nil    ; documentHighlight per move
        lsp-headerline-breadcrumb-enable nil  ; documentSymbol per move
        lsp-eldoc-enable-hover nil            ; hover per move; use `, h h'
        lsp-enable-dap-auto-configure nil     ; don't pull in dap-mode at all
        lsp-idle-delay 0.75)

  ;; Cheap stand-in for the breadcrumb: same orientation, no LSP round-trip.
  (which-function-mode 1)
  (spaceline-toggle-which-function-on)

  (load (expand-file-name "private/local/lsp-render-filters.el"
